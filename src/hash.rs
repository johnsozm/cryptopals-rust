//Trait for hash functions to implement
trait Hash {
    fn digest(message: &Vec<u8>) -> Vec<u8>;
}
...
mod tests {
    ...
    #[test]
    fn test_sha1() {
        assert_eq!(bytes_to_hex(&SHA1::digest(&ascii_to_bytes("abc"))), "a9993e364706816aba3e25717850c26c9cd0d89d");
        assert_eq!(bytes_to_hex(&SHA1::digest(&ascii_to_bytes(""))), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
        assert_eq!(bytes_to_hex(&SHA1::digest(&ascii_to_bytes("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"))), "84983e441c3bd26ebaae4aa1f95129e5e54670f1");
        assert_eq!(bytes_to_hex(&SHA1::digest(&ascii_to_bytes("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu"))), "a49b2446a02c645bf419f995b67091253a04a259");
        assert_eq!(bytes_to_hex(&SHA1::digest(&ascii_to_bytes(&"a".repeat(1000000)))), "34aa973cd4c4daa4f61eeb2bdbad27316534016f");
    }
}